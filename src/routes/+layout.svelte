...
<div class="relative flex flex-col max-w-[1400px] mx-auto w-full text-sm sm:text-base min-h-screen">
    <div class={"fixed bottom-0 w-full duration-200 flex p-10 z-[10]" +(
            y > 0 ? 'opacity-full pointer-events-auto' : 'pointer-events-none opcacity-0'
        )
    }>
        <button on:click={goTop()} class="ml-auto rounded-full bg-slate-900 px-3 sm:px-4 hover:bg-slate-800 cursor-pointer">
